...
gnome-terminal --window -- bash -c "cd $BUILD_DIR && cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug .. && gnome-terminal --tab -- bash -c \"cmake --build . && gnome-terminal --tab -- bash -c \\\"gdb ./$APPLICATION_BIN; exit_code=\\\\\\\$?; echo; echo; echo; echo -e \\\\\\\"\033[1;31mProcess finished with exit code \\\\\\\$exit_code\033[0m\\\\\\\"; echo; exec bash;\\\"; exec bash;\"; exec bash;"
